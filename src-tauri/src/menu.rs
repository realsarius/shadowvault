...
pub fn build_menu(app: &AppHandle, lang: &str) -> tauri::Result<Menu<tauri::Wry>> {
    ...
    let view_sub = Submenu::with_items(
        ...
        true,
        &[
            &nav_dash, &nav_src, &nav_logs, &nav_lic, &nav_set, &sep_v1, &sidebar,
        ],
    )?;
    ...
    let win_sub = Submenu::with_items(app, l.window, true, &[&minimize, &sep_w1, &bring_all])?;
